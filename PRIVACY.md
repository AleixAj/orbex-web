...
### 2.6 Messages you send us (support and reports)

Two screens in the game let you write to us. Both are entirely optional — nothing is sent unless you type it yourself and press send.

- **Settings → Contact**: a subject (up to 120 characters) and a message (up to 2,000). Along with it the game sends the technical context needed to help you — app version, platform and game language — and the form tells you so on screen before you send it.
- **Report a player** (from their profile in the ranking): a reason picked from a fixed list, the nickname of the player you are reporting, the leaderboard you saw them on, and an optional note of up to 280 characters.

What happens to that text:

- **Only I read it.** It is never shown to other players, and a reported player is never told who reported them or what the note said.
- A contact message is answered **inside the game**, in your mailbox.
- So that a message arriving in a language I don't speak can be read, and the reply sent back in yours, the text is passed to a translation provider — see section 6.
- It is deleted **90 days** after being answered or resolved (section 8).

Because it is free-form text, **please don't include personal details you don't need to give us**: an address, a phone number or a password is never necessary for a support message.
...
- **Answering you and moderating the game**: replying to your support messages, and reviewing reports about other players (2.6).
  *Legal basis: performance of the service for support (GDPR art. 6.1.b), legitimate interest for moderation (GDPR art. 6.1.f).*
- **Personalised advertising** (section 3).
  *Legal basis: your consent — Google's UMP form (GDPR art. 6.1.a).*

**We do NOT use your data for**: marketing emails, profiling you as a person, training AI models, or selling to third parties. One nuance: the translation provider in 2.6 may use the text of a message you send us to improve its own service — that is its processing, not ours, and all it ever sees is what you typed.
...
- **DeepL** (DeepL SE, Germany): translates the support messages and reports of section 2.6 — the text you wrote and nothing else, never your nickname, your email or your identifier. We use DeepL's free API tier, whose terms allow DeepL to use the submitted texts to improve its own translation service; that is why the section above asks you not to write anything you don't need to. No other part of the game is sent to it. DeepL's privacy policy: https://www.deepl.com/privacy
...
- **Support messages and reports** live at Supabase alongside everything else. When one of them is translated, its text passes through DeepL's servers (DeepL SE, Germany — European Union).
...
- **Server**: indefinitely while your account is active, with these exceptions:
  ...
  - **Support messages and reports** (2.6) are deleted by a scheduled job **90 days** after being answered or resolved. Anything still open is never deleted by age — that is precisely what nobody has dealt with yet.
...
When that happens we record the reason and the date. Sanctions can be temporary or permanent, they are always reversible on our side, and you can appeal any of them straight from the notice the game shows you — it opens the contact form with the subject already filled in — or by writing to playorbex@gmail.com.
...
You can also write from inside the game, at **Settings → Contact**; the reply arrives in your in-game mailbox.
...
**Vigente desde:** 24 de agosto de 2026
...
### 2.6 Mensajes que nos escribes (soporte y denuncias)

Hay dos pantallas del juego desde las que puedes escribirnos. Las dos son totalmente opcionales: no se envía nada salvo que lo escribas tú y pulses enviar.

- **Opciones → Contacto**: un asunto (hasta 120 caracteres) y un mensaje (hasta 2.000). Con él viaja el contexto técnico necesario para poder ayudarte — versión de la app, plataforma e idioma del juego —, y el propio formulario te lo dice en pantalla antes de enviarlo.
- **Denunciar a un jugador** (desde su perfil en el ranking): un motivo elegido de una lista cerrada, el apodo del jugador al que denuncias, el tablero en el que lo viste y una nota opcional de hasta 280 caracteres.

Qué pasa con ese texto:

- **Lo leo solo yo.** No se enseña a otros jugadores, y a un jugador denunciado nunca se le dice quién lo ha denunciado ni qué decía la nota.
- Un mensaje de contacto se responde **dentro del juego**, en tu buzón.
- Para poder leer un mensaje que llega en un idioma que no hablo, y para devolverte la respuesta en el tuyo, el texto se pasa a un proveedor de traducción — ver apartado 6.
- Se borra a los **90 días** de haberse respondido o resuelto (apartado 8).

Como es texto libre, **no incluyas datos personales que no necesites darnos**: para un mensaje de soporte nunca hace falta una dirección, un teléfono ni una contraseña.
...
- **Atenderte y moderar el juego**: responder a tus mensajes de soporte y revisar las denuncias sobre otros jugadores (2.6).
  *Base legal: ejecución del servicio para el soporte (RGPD art. 6.1.b) e interés legítimo para la moderación (RGPD art. 6.1.f).*
- **Publicidad personalizada** (apartado 3).
  *Base legal: tu consentimiento — el formulario UMP de Google (RGPD art. 6.1.a).*

**NO usamos tus datos para**: correos comerciales, perfilarte como persona, entrenar modelos de IA o venderlos a terceros. Con un matiz: el proveedor de traducción del apartado 2.6 puede usar el texto de un mensaje que nos envíes para mejorar su propio servicio — eso es tratamiento suyo y no nuestro, y lo único que ve es lo que hayas escrito.
...
- **DeepL** (DeepL SE, Alemania): traduce los mensajes de soporte y las denuncias del apartado 2.6 — el texto que escribiste y nada más, nunca tu apodo, tu correo ni tu identificador. Usamos su API gratuita, cuyos términos permiten a DeepL usar los textos enviados para mejorar su propio servicio de traducción; por eso el apartado anterior te pide que no escribas nada que no haga falta. No se le envía ninguna otra parte del juego. Política de privacidad de DeepL: https://www.deepl.com/privacy
...
- **Los mensajes de soporte y las denuncias** se guardan en Supabase con todo lo demás. Cuando se traduce uno, su texto pasa por los servidores de DeepL (DeepL SE, Alemania — Unión Europea).
...
- **Servidor**: indefinidamente mientras tu cuenta esté activa, con estas excepciones:
  ...
  - **Mensajes de soporte y denuncias** (2.6): una tarea programada los borra a los **90 días** de responderse o resolverse. Lo que sigue abierto no se borra por antigüedad — es justo lo que nadie ha atendido todavía.
...
Cuando eso ocurre registramos el motivo y la fecha. Las sanciones pueden ser temporales o permanentes, siempre son reversibles por nuestra parte, y puedes recurrir cualquiera de ellas desde el propio aviso que te enseña el juego —que abre el formulario de contacto con el asunto ya puesto— o escribiendo a playorbex@gmail.com.
...
También puedes escribirme desde dentro del juego, en **Opciones → Contacto**; la respuesta te llega al buzón.
